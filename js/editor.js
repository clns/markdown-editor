...
var saveSelection = $window.cledit.Utils.debounce(function () {
    ...
    if (range) {
        if (!clEditorSvc.previewElt || !(clEditorSvc.previewElt.compareDocumentPosition(range.startContainer) & $window.Node.DOCUMENT_POSITION_CONTAINED_BY) || !(clEditorSvc.previewElt.compareDocumentPosition(range.endContainer) & $window.Node.DOCUMENT_POSITION_CONTAINED_BY)
        ) {
            range = undefined
        }
    }
